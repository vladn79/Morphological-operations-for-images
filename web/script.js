    ...
    function uploadFile() {
      ...
      xhr.onload = function () {
          if (xhr.status === 200) {
              ...
              var imageUrl = response['url']; // Отримати посилання на картинку з відповіді сервера
              submitInput(imageUrl); // Викликати функцію submitInput() з посиланням на картинку
          } 
      };
      xhr.send(formData);
  }
  
  function submitInput(imageUrl) {
      var shapeValue = document.getElementById("shapeInput").value;
      var resultValue = document.getElementById("result").innerText;
      var result1Value = document.getElementById("result1").innerText;
      eel.add_result_to_array(shapeValue, resultValue, result1Value, imageUrl); // Передача посилання на картинку до Python-коду
      document.getElementById("result1").innerText = shapeValue;
      document.getElementById("inputPopup").style.display = "none";
  }
    ...
    eel.get_results_array()().then(displayResultsArray);
});
